...
app.get('/',(_q,s)=>s.send('<h1>Editor Automático V7.2</h1><p>Servidor FFmpeg nativo.</p><p><a href="/health">/health</a></p>'));
app.get('/health',(_q,s)=>s.json({ok:true,service:'editor-automatico-ffmpeg',version:'v7.2',ffmpeg:'native',time:new Date().toISOString()}));
...
server=app.listen(PORT,'0.0.0.0',()=>console.log(`FFmpeg server V7.2 listening on ${PORT}`));
